<?php
    ...
    if ($add_user && $size > 0) {
        if (move_uploaded_file($tmp_path, $_SERVER['DOCUMENT_ROOT'] . $path)) {
            $str_spec = "INSERT INTO `blog-item`(`name`, `description`, `photo`, `created_at`, `updated_at`) VALUES ('$name_c','$desc','$full_name',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)";
            
            $run_spec = mysqli_query($connect, $str_spec);
            if ($run_spec) {
                header("Location:/adminka_blog.php");
                exit();
            } else {
